...
- `PB_DOMAIN` — PocketBase API + admin UI (예: `pb.<your>.com` / `pb-climb-forge.duckdns.org`)
- `APP_DOMAIN` — 프론트 정적 자산 (예: `app.<your>.com` / `app-climb-forge.duckdns.org`)
...
2. 빈 칸에 원하는 서브도메인 2개 입력 → `add domain`:
   - 예: `climb-forge-pb`, `climb-forge-app`
3. 페이지 상단의 **token** 복사 (한 토큰이 모든 서브도메인 관리).
4. **현재 IP를 두 서브도메인에 동시 등록**:

   ```bash
   # SUBDOMAIN을 콤마로 묶으면 한 번 호출로 둘 다 갱신.
   curl "https://www.duckdns.org/update?domains=climb-forge-pb,climb-forge-app&token=<TOKEN>&ip=<VM 외부 IP>"
   # → "OK" 응답이면 성공
   ```
...
5. 전파 확인:
   ...
   ```bash
   dig +short climb-forge-pb.duckdns.org
   dig +short climb-forge-app.duckdns.org
   ```
...
```
PB_DOMAIN=climb-forge-pb.duckdns.org
APP_DOMAIN=climb-forge-app.duckdns.org
...
```bash
# crontab -e
0 4 1 * * curl -fsS "https://www.duckdns.org/update?domains=climb-forge-pb,climb-forge-app&token=<TOKEN>&ip=" >/dev/null
```
...
```bash
sudo mkdir -p /opt/climb-forge/data/{pb_data,caddy_data,caddy_config,app}
...
sudo chown -R "$USER:$USER" /opt/climb-forge/data/app
```
...
vi .env   # PB_DOMAIN, APP_DOMAIN, ACME_EMAIL, DATA_DIR=/opt/climb-forge/data 채움
```
...
rsync -avz --delete out/ \
  "<vm-user>@<VM-IP>:/opt/climb-forge/data/app/"
```
...
| `DEPLOY_PATH` | `/opt/climb-forge/data/app` |
...
```bash
ssh-keygen -t ed25519 -N "" -f ~/.ssh/climb-forge-deploy -C "github-actions-deploy"
# public key를 VM의 authorized_keys에 추가
ssh-copy-id -i ~/.ssh/climb-forge-deploy.pub <user>@<VM-IP>
# private key 전문을 GitHub Secret DEPLOY_SSH_KEY에 등록
cat ~/.ssh/climb-forge-deploy
```
