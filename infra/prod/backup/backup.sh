#!/bin/bash
# Breakteau 일일 백업 (S14, ADR-6 — GCS).
#
# 흐름:
#   1) PB admin 토큰 발급 (PB API)
#   2) PB 측에서 일관 zip 생성 (`POST /api/backups`) — WAL 일관성 보장
#   3) zip 다운로드 → 무결성 검증 → GCS 업로드 (rclone, S3 호환 + HMAC)
#   4) PB 서버 측 zip 정리 (디스크 절약)
#   5) GCS 30일 이전 객체 cleanup
#
# 인증:
#   GCS HMAC access key/secret을 환경변수로 노출 (rclone S3 backend, provider=GCS).
#   bucket-scoped SA + 백업/미디어 별 HMAC 분리 권장 (RUNBOOK §7.3).
#
...
: "${RCLONE_CONFIG_GCS_ACCESS_KEY_ID:?GCS HMAC access key required (rclone env)}"

# 자동 백업이 GCS에 격리될 sub-prefix.
# 미디어(v1.1)는 별도 버킷(`breakteau-media`) + 별도 SA/HMAC 가정 → cleanup 영향 분리.
...
# ── 4) GCS 업로드 (sub-prefix auto/로 격리 — 미디어 객체와 cleanup 영향 분리)
