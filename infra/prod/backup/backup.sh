...
# 자동 백업이 R2에 격리될 sub-prefix. 동일 버킷의 다른 객체와 cleanup 영향 분리.
...
# ── 4) R2 업로드 (sub-prefix auto/로 격리 — 다른 객체와 cleanup 영향 분리)
